...
#define OUTERD_LIB_PATH "libouterd.so"
typedef bool (*OuterDCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_outerd_core_main() {
   void* libouterd = dlopen(OUTERD_LIB_PATH, RTLD_LAZY);
   if (!libouterd) {
      fprintf(stderr,"Failed to load \"libouterd.so\"\n");
      char* error;
   ...
   auto core_main = (OuterDCoreMain) dlsym(libouterd,"outerd_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"outerd_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
