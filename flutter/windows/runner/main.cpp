...
typedef char** (*FUNC_OUTERD_CORE_MAIN)(int*);
typedef void (*FUNC_OUTERD_FREE_ARGS)( char**, int);
typedef int (*FUNC_OUTERD_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libouterd.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libouterd.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_OUTERD_CORE_MAIN outerd_core_main =
      (FUNC_OUTERD_CORE_MAIN)GetProcAddress(hInstance, "outerd_core_main_args");
  if (!outerd_core_main)
  {
    std::cout << "Failed to get outerd_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_OUTERD_FREE_ARGS free_c_args =
      (FUNC_OUTERD_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = outerd_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "OuterD [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"OuterD";
  FUNC_OUTERD_GET_APP_NAME get_outerd_app_name = (FUNC_OUTERD_GET_APP_NAME)GetProcAddress(hInstance, "get_outerd_app_name");
  if (get_outerd_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_outerd_app_name(app_name_buffer, 512) == 0) {
