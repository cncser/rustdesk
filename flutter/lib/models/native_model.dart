...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libouterd.so')
        : isLinux
            ? DynamicLibrary.open('libouterd.so')
            : isWindows
                ? DynamicLibrary.open('libouterd.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibouterd.dylib") :
  ...
  void _startListenEvent(RustdeskImpl outerdImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = outerdImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
