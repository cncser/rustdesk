...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_outerd() {
            if s.contains("OuterD")
                && !name.starts_with("upgrade_outerd_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("OuterD") {
                    s = s.replace("OuterD", &app_name);
                } else {
                    // https://github.com/outerd/outerd-server-pro/issues/845
                    // If app_name contains "OuterD" (e.g., "OuterD-Admin"), we need to avoid
                    // replacing "OuterD" within the already-substituted app_name, which would
                    // cause duplication like "OuterD-Admin" -> "OuterD-Admin-Admin".
                    //
