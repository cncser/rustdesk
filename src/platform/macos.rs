...
fn get_update_temp_dir() -> PathBuf {
    ...
    Path::new("/tmp").join(format!(".outerdupdate-{}", euid))
}
...
// remove just one app from all the permissions: tccutil reset All com.carriez.outerd
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.carriez.outerd", &bundleid);
    }
    s = s.replace("outerd", &crate::get_app_name().to_lowercase());
    s = s.replace("OuterD", &crate::get_app_name());
    s
}
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/OuterD.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another OuterD.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // OuterD.app/Contents/MacOS/OuterD
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/OuterDUpdate";
