// https://github.com/outerd/outerd/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteOuterDTestCertsW_SingleHive()
//
//  Purpose:    Deletes OuterD Test certificates and wrong key stores
//
...
BOOL DeleteOuterDTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteOuterDTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteOuterDTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteOuterDTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteOuterDTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
