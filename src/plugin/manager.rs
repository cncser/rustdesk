...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "outerd".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/outerd-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "OuterD wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "OuterD wants to uninstall the plugin",
    )
}
