...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "OUTERD_APPNAME";
...
// Because the url is always `https://api.outerd.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_OUTERD_CLIENT.to_string());
...
#[inline]
pub fn is_outerd() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("OuterD")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.outerd.com".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    let url = url.to_ascii_lowercase();
    url.contains("outerd.com/") || url.ends_with("outerd.com")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/OuterD.app/Contents/MacOS/OuterD", "XPC_SERVICE_NAME=com.carriez.OuterD_server"
...
pub type OuterDInterval = ThrottledInterval;

#[inline]
pub fn outerd_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    async fn test_OuterD_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = outerd_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_OuterD_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "outerd.com/"
        assert!(is_public("https://outerd.com/"));
        assert!(is_public("https://www.outerd.com/"));
        assert!(is_public("https://api.outerd.com/v1"));
        assert!(is_public("https://API.OUTERD.COM/v1"));
        assert!(is_public("https://outerd.com/path"));

        // Test URLs ending with "outerd.com"
        assert!(is_public("outerd.com"));
        assert!(is_public("https://outerd.com"));
        assert!(is_public("https://OuterD.com"));
        assert!(is_public("http://www.outerd.com"));
        assert!(is_public("https://api.outerd.com"));
        ...
        assert!(!is_public("https://outerd.computer.com"));
        assert!(!is_public("outerd.comhello.com"));
    }
    ...
    fn test_should_use_tcp_proxy_for_api_url() {
        ...
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.outerd.com/api/login",
            "https://admin.outerd.com"
        ));
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.example.com/api/login",
            "not a url"
